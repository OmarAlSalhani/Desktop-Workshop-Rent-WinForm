...
namespace app7.products_forms
{
    public partial class ar_products_form : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void save_btn_Click(object sender, EventArgs e)
        {
            ...
            {
                OmarNotifications.Alert.ShowInformation("الرجاء أدخال أسم المادة");
                return;
            }
            if (Convert.ToDouble(daily_rent_tb.Text) <= 0)
            {
                OmarNotifications.Alert.ShowInformation("الرجاء أدخال سعر المادة");
                return;
            }
        ...
        private void update_btn_Click(object sender, EventArgs e)
        {
            ...
            {
                OmarNotifications.Alert.ShowInformation("يجب تحديد مادة واحدة فقط");
                return;
            }
        ...
        private void delete_btn_Click(object sender, EventArgs e)
        {
            ...
            if (main_gridview.SelectedRowsCount > 0)
            {
                ...
                DialogResult dr = classes.notifications_class.my_messageBox("هل تريد حذف المواد المحددة ؟", MessageBoxButtons.YesNo, MessageBoxIcon.Information);
                if (dr == DialogResult.Yes)
                {
                    foreach (int i in main_gridview.GetSelectedRows())
                    {
                        ...
                        logs_class.log_add($"حذف المادة ذات الرقم {id_tb.Text}", 0, "المواد");
                    }
        ...
        {
            get_data_table = connection_class.select("select id as 'رقم المادة' , product_name as 'أسم المادة',daily_rent as 'السعر قبل الضريبة' from products_table order by id desc");
        }
