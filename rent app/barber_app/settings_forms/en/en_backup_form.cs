...
namespace app7.n_settings_forms.en
{
    ...
    public partial class en_backup_form : DevExpress.XtraEditors.XtraForm
    {
        public en_backup_form()
        {
            InitializeComponent();
        }
        ...
        private void db_folder_text_Properties_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            if (folderBrowserDialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                {
                    classes.notifications_class.my_messageBox($"Cannot create backup on C", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
        ...
        private void add_to_backup_grid_btn_Click(object sender, EventArgs e)
        {
            ...
            {
                OmarNotifications.Alert.ShowInformation("Select Backup folder");
                return;
            }
            ...
            for (int i = 0; i < check_table.Rows.Count; i++)
            {
                ...
                {
                    OmarNotifications.Alert.ShowInformation("Schedualed before");
                    return;
                }
            }
        ...
        {
            DataTable table = connection_class.select($"select id as 'ID',the_time as 'Time',the_folder as 'Folder' from backup_scheduale_table");
        ...
        private void en_backup_form_Load(object sender, EventArgs e)
        {
